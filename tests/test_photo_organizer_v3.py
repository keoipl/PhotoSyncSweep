...
import photo_organizer_v3 as organizer

from photo_organizer_v3 import (
    ACTION_COPY,
    ACTION_MOVE,
    BUILTIN_PRESETS,
...
class PhotoOrganizerV2Tests(unittest.TestCase):
    ...
    def test_language_switch_state_carries_settings_into_chinese(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            try:
                state_path.write_text(
                    json.dumps(
                        {
                            "target_language": "zh",
                            "reference_folder": "C:/照片",
                            "target_folder": "C:/照片",
                            "quarantine_folder": "D:/暂存",
                            "same_folder": True,
                ...
                result = organizer.apply_language_switch_state({}, "zh")
                self.assertEqual(result["mode"], organizer.MODE_JPG_TO_RAW)
                self.assertEqual(result["operation"], organizer.ACTION_MOVE)
                self.assertEqual(result["target_extensions"], ".ARW")
